-- Enforce legal signature name from authenticated account profile.
...
begin
  ...
  if v_profile_full_name is null then
    raise exception 'full name missing on account profile';
  end if;
  ...
  if v_role = 'provider' then
    ...
    set
      provider_signature_name = coalesce(provider_signature_name, v_profile_full_name),
    ...
    update public.booking_contracts
    set
      client_signature_name = coalesce(client_signature_name, v_profile_full_name),
